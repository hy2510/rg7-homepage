...
import style from "./global-option.module.scss";
...
// My RG > 남은기간
const MyRgUseEndDate = ({ useEndDate }) => {
  return (
...
// My RG > 과제정보
const MyRgAssignmentInfo = ({ onClickTodo, onClickFavorite }) => {
  return (
      ...
      <div className={style.assignment_info_container}>
        <Link href="/library/find/assignment/to-do" onClick={onClickTodo}>
          <div className={style.to_do}>
            ...
            <div className={style.count_num}>150</div>
          </div>
        </Link>
        <Link
...
// My RG > 학습목표
const MyRgGoalInfo = ({ onClickDailyGoalSetting, onClickMyStudyLevel }) => {
  return (
...
// 나의 프로필 > 어워드 리스트
const AwardListContainer = ({ children }) => {
  return <div className={style.award_list_container}>{children}</div>;
};

// 나의 프로필 > 어워드 아이템
const AwardListItem = ({ text, collectNum, onClick }) => {
  return (
    <div className={style.award_list_item} onClick={onClick}>
      <div className={style.col_a}>
        <span className={style.txt_l}>어워드</span>
...
// 프로필 수정
export function EditProfile({ onClickChangeAvatar }) {
  return (
...
// 아바타 수정 > 아바타 아이템
const AvatarItem = ({ selected, check, avtImgSrc, avtName }) => {
  return (
    <div className={style.avatar_item}>
      ...
      >
        <div
          ...
          style={{ backgroundImage: `url(${avtImgSrc})` }}
        ></div>
      </div>
...
export function DailyGoalAward() {
  ...
  return (
    <div className={style.daily_goal_award}>
      <AlertBar>
        일일 목표를 완료하여 25일 마다 새로운 어워드를 획득해 보세요.
      </AlertBar>
      {emptyMessage ? (
...
const DailyGoalAwardItem = ({ awardImgSrc, awardName, awardGetDate }) => {
  return (
...
const StreakAwardItem = ({ awardImgSrc, awardName, awardGetDate }) => {
  return (
...
const ChallengeAwardItem = ({ awardImgSrc, awardName, awardGetDate }) => {
  return (
...
// 레벨마스터 어워드 > 아이템
const LevelMasterAwardItem = ({ awardImgSrc, awardGetDate, awardLevel }) => {
...
export function DailyGoalSetting() {
  ...
  return (
    <div className={style.daily_goal_setting}>
      ...
      <div className={style.row_b}>
        <div className={style.txt_h}>일일 목표 설정</div>
...
const DailyGoalSetHistoryItem = ({ date, comment, goal }) => {
  return (
            ...
            return (
              <div>
                <LeveledStudyStatusItem
                  ...
                  levelMaster={a.master}
                  levelMasterImage={a.masterImage}
                  studentLevel={currentLevel == i && "Y"}
                />
              </div>
            );
          })}
    ...
    <div className={style.leveled_study_status_item}>
      {levelMaster == "Y" ? (
        <div
          ...
          style={{ backgroundImage: `url(${levelMasterImage})` }}
        ></div>
      ) : (
        <div
...
const LevelTestHistoryItem = ({ testResultLevel, testDate, onClick }) => {
  return (
...
// 학습 설정 > 학습 옵션 설정 아이템
const SetStudyOptionItem = ({ check, title, discription }) => {
  return (
